...
const Contact = () => {
  ...
  const { data: contactInfo } = useSupabaseData(() => db.contactInfo(), []);

  // Use Supabase data if available, otherwise fall back to translations
  const phone = contactInfo?.phone_primary || t('contact.phoneValue');
  const phoneSecondary = contactInfo?.phone_secondary || '';

  // Split multiple emails if they exist (comma-separated)
  const emailData = contactInfo?.email || t('contact.emailValue');
  const emailList = emailData.split(',').map(e => e.trim()).filter(e => e);
  const email = emailList.join(' / '); // Display with / separator

  const address = contactInfo?.address || t('contact.locationValue');
            ...
            <div className="space-y-6">
              {[
                ...
                },
                {
                  ...
                  content: phoneSecondary ? `${phone} / ${phoneSecondary}` : phone,
                },
                {
                  icon: Mail,
