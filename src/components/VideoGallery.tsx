...
const VideoGallery = () => {
  ...
  const { data: videosData } = useSupabaseData(() => db.videos(), []);

  // Use Supabase data if available, otherwise fall back to translations
  const videos = videosData && videosData.length > 0
    ? videosData.map((vid: any, index: number) => ({
        id: index + 1,
        title: vid.title,
        thumbnail: `${vid.thumbnail_gradient_from} to ${vid.thumbnail_gradient_to}` || 'from-green-600 to-green-500',
