...
const About = () => {
  ...
  const { data: aboutSection } = useSupabaseData(() => db.aboutSection(), []);
  const { data: highlightsData } = useSupabaseData(() => db.highlights(), []);

  // Use Supabase data if available, otherwise fall back to translations
  const highlights = highlightsData && highlightsData.length > 0
    ? highlightsData.map((h: any) => h.title)
    : [
        t('about.highlight1'),
        t('about.highlight2'),
        t('about.highlight3'),
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-50 rounded-full mb-4">
            ...
            <span className="text-body-sm font-bold text-green-700 uppercase tracking-widest">
              {aboutSection?.badge_text || t('about.badge')}
            </span>
          </div>
          <h2 className="text-h1 md:text-display font-bold text-gray-900 mb-4">
            {aboutSection?.main_heading || t('about.title')}
            <span className="block mt-2 bg-gradient-to-r from-green-600 via-cyan-600 to-blue-600 bg-clip-text text-transparent">
              {aboutSection?.gradient_heading || t('about.titleHighlight')}
            </span>
          </h2>
          <p className="text-body-lg md:text-h4 text-gray-600 max-w-3xl mx-auto">
            {aboutSection?.subtitle || t('about.subtitle')}
          </p>
        </div>
