...
const CoreValues = () => {
  ...
  // Fetch core values from Supabase
  const { data: coreValuesData } = useSupabaseData(() => db.coreValues(), []);

  // Icon mapping
  const iconMap: Record<string, any> = {
    Heart, MessageCircle, TrendingUp, Shield, Target
  };

  // Use Supabase data if available, otherwise fall back to translations
  const values = coreValuesData && coreValuesData.length > 0
    ? coreValuesData.map((item: any) => ({
        icon: iconMap[item.icon_name] || Heart,
        title: item.title,
