...
const Commitments = () => {
  ...
  const { data: commitmentsData } = useSupabaseData(() => db.commitments(), []);

  // Icon mapping
  const iconMap: Record<string, any> = {
    CheckCircle, Users, Award, TrendingDown
  };

  // Use Supabase data if available, otherwise fall back to translations
  const commitments = commitmentsData && commitmentsData.length > 0
    ? commitmentsData.map((item: any) => ({
